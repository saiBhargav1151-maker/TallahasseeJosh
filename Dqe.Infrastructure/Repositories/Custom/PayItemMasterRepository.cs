...
namespace Dqe.Infrastructure.Repositories.Custom
{
    public class PayItemMasterRepository : BaseRepository, IPayItemMasterRepository
    {
        ...
        public IEnumerable<PayItemMaster> GetFixedPriceItems(Domain.Model.Wt.Project p)
        {
            InitializeSession();
            ...
            var currentYear = sy.Substring(1);
            var specYear = p == null ? currentYear : string.IsNullOrWhiteSpace(p.SpecBook) ? currentYear : p.SpecBook;
            var disjunction = new Disjunction();
            disjunction.Add(Restrictions.Where<PayItemMaster>(i => i.IsFixedPrice));
            disjunction.Add(Restrictions.Where<PayItemMaster>(i => i.BidRequirementCode == "Fixed"));
            return Session
                    .QueryOver<PayItemMaster>()
                    .Where(disjunction)
                    .JoinQueryOver(i => i.MyMasterFile)
