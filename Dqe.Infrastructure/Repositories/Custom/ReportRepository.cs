...
namespace Dqe.Infrastructure.Repositories.Custom
{
    public class ReportRepository : BaseRepository, IReportRepository
    {
        ...
        public void RebuildReportStructure(Letting letting, List<ReportProposal> officialProposals, List<ReportProposal> authorizedProposals, bool rebuildVendorBidData, List<Domain.Model.PayItemMaster> frontLoadedPayItems)
        {
            RebuildReportStructureForAuthorization(letting, authorizedProposals);

            RebuildReportStructureForOfficial(letting, officialProposals, rebuildVendorBidData, frontLoadedPayItems);
        }

        public void RebuildReportStructureForOfficial(Letting letting, List<ReportProposal> officialProposals, bool rebuildVendorBidData, List<Domain.Model.PayItemMaster> frontLoadedPayItems)
        {
            InitializeSession();
            ...
            foreach (var proposal in reportLetting.ReportProposals)
            {
                ...
                if (rebuildVendorBidData && wtLettingProposal.ProposalVendors.Any() && wtLettingProposal.ProposalVendors.Any(b => b.Bids.Any()))
                {
                    ...
                    {
                        SetLowBid(proposal);
                        DetermineUnbalancedItems(proposal, frontLoadedPayItems);
        ...
        private static void DetermineUnbalancedItems(ReportProposal proposal, List<Domain.Model.PayItemMaster> frontLoadedPayItems)
        {
            ...
            foreach (var proposalItem in proposal.ReportProposalItems)
            {
                ...
                {
                    if (frontLoadedPayItems.Any(i => i.RefItemName == item.MyReportProposalItem.ItemNumber))
                        item.Tolerance = "F";
                    else
                        item.Tolerance = "A";
                }
            ...
            foreach (var milestone in proposal.ReportProposalMilestones.Where(m => m.CostPerDay > 0))
            {
                //var unitPriceBidAverage = (milestone.Total + milestone.ReportMilestoneBids.Sum(i => i.BidPrice * i.NumberOfDaysBid)) /
                ...
                //AverageMilestonePriceByBid(milestone, unitPriceBidAverage, reportMilestoneBid);
                AverageMilestonePriceByBid(milestone, (int)averageDaysBid, reportMilestoneBid);

                //var unbalanced = milestone.ReportMilestoneBids
                //    .Where(i => i.BidPrice * i.NumberOfDaysBid < milestone.AverageBidPrice * (decimal).4)
                //    .ToList();
                //var aboveTolerance = milestone.ReportMilestoneBids
                //    .Where(i => i.BidPrice * i.NumberOfDaysBid > milestone.AverageBidPrice * (decimal)1.6)
                //    .ToList();
                var unbalanced = milestone.ReportMilestoneBids
        ...
        //private static void AverageMilestonePriceByBid(ReportProposalMilestone reportProposalMilestone, decimal unitPriceBidAverage, ReportMilestoneBid reportMilestoneBid)
        //{
        //    var l = reportProposalMilestone.ReportMilestoneBids.ToList();
        //    l.Add(reportMilestoneBid);

        //    var firstCheck = l
        //        .Where(i => i.BidPrice * i.NumberOfDaysBid > unitPriceBidAverage * (decimal).55)
        //        .Where(i => i.BidPrice * i.NumberOfDaysBid < unitPriceBidAverage * (decimal)1.45)
        //        .ToList();

        //    if (firstCheck.Any())
        //    {
        //        reportProposalMilestone.AverageBidPrice = firstCheck.Sum(i => i.BidPrice) / firstCheck.Count;
        //        return;
        //    }

        //    var secondCheck = l
        //        .Where(i => i.BidPrice * i.NumberOfDaysBid > unitPriceBidAverage * (decimal).1)
        //        .Where(i => i.BidPrice * i.NumberOfDaysBid < unitPriceBidAverage * (decimal)1.9)
        //        .ToList();

        //    if (secondCheck.Any())
        //    {
        //        reportProposalMilestone.AverageBidPrice = secondCheck.Sum(i => i.BidPrice) / secondCheck.Count;
        //        return;
        //    }

        //    var thirdCheck = l
        //        .Where(i => i.BidPrice * i.NumberOfDaysBid > unitPriceBidAverage * 0)
        //        .Where(i => i.BidPrice * i.NumberOfDaysBid < unitPriceBidAverage * 2)
        //        .ToList();

        //    if (thirdCheck.Any())
        //        reportProposalMilestone.AverageBidPrice = thirdCheck.Sum(i => i.BidPrice) / thirdCheck.Count;
        //}
        ...
        private static void DetermineMagnitude(ReportProposal reportProposal)
        {
            ...
            {
                milestone.Magnitude = Math.Abs(milestone.Total - milestone.ReportMilestoneBids.Min(i => i.BidPrice * i.NumberOfDaysBid));
            }
        }
        ...
        private static void GenerateLettingSummary(ReportLetting reportLetting)
        {
            ...
            for (var j = 0; j <= 5; j++)
            {
                //TODO when B portion is fixed.  We need to add the b sums to the total
                ...
                if (proposals.Any())
                {
                    ...
                    reportLettingSummary.ValueInCategory = proposals.Sum(p => p.ReportProposalVendors.Min(v => v.Total)); //Sum(v => v.ReportVendorBids.Where(b => b.LowCost).Sum(b => b.BidPrice)));
                    ...
                    reportLettingSummary.PercentageOfLettingTotal = Math.Round((reportLettingSummary.ValueOfEstimate / (reportLetting.Total)) * 100, 2); //+ reportLetting.ReportProposals.Sum(i => i.ReportProposalMilestones.Sum(m => m.Total)))) * 100, 2);
                    ...
                    {
                        var bidRange = (from vendor in item.ReportProposalVendors
                                        where item.ReportProposalVendors.Any()
                                        select Math.Round(((vendor.Total / (item.Total)) * 100) - 100, 2)) //+ item.ReportProposalMilestones.Sum(m => m.Total))) * 100) - 100, 2))
                                        .ToList();
                    ...
                    reportLettingSummary.AverageBidsPerContract = Math.Round(proposals.Sum(i => i.ReportProposalVendors.Sum(v => v.ReportVendorBids.Count()) / (decimal)reportLettingSummary.NumberOfContracts), 2);
                }
                else
                {
