                    ...
                    if (vendor.Bids.Any())
                    {
                        ...
                        foreach (var bid in vendor.Bids)
                        {
                            ...
                            if (myProposalItem != null)
                            {
                                ...
                                {
                                    BidPrice = bid.BidPrice != null ? myProposalItem.Unit.StartsWith("LS") ? (decimal)bid.BidPrice / myProposalItem.Quantity : (decimal)bid.BidPrice : 0,
                                    LowCost = bid.LowCost
                                };
        ...
        private static void DetermineUnbalancedItems(ReportProposal proposal)
        {
            foreach (var proposalItem in proposal.ReportProposalItems)
            {
                ...
                var unbalanced = proposalItem.ReportVendorBids
                    .Where(i => i.BidPrice < unitPriceBidAverage * (decimal).4)
                    .ToList();
                var aboveTolerance = proposalItem.ReportVendorBids
                    .Where(i => i.BidPrice > unitPriceBidAverage * (decimal)1.6)
                    .ToList();
        ...
        private static void GenerateLettingSummary(ReportLetting reportLetting)
        {
            ...
            for (var j = 0; j <= 5; j++)
            {
                ...
                if (proposals.Any())
                {
                    ...
                    reportLettingSummary.PercentageOfContracts = Math.Round((proposals.Count() / (decimal)reportLetting.ReportProposals.Count()), 2);
                    reportLettingSummary.PercentageOfLettingTotal = Math.Round(reportLettingSummary.ValueOfEstimate / reportLetting.Total, 2);
