...
namespace Dqe.Infrastructure.Repositories.Custom
{
    public class ProposalRepository : BaseRepository, IProposalRepository
    {
        ...
        public void DeleteProposalStructure(int id)
        {
            InitializeSession();
        ...
        public void BuildReportProposal(int proposalId, DqeUser owner, IPayItemMasterRepository payItemMasterRepository, IWebTransportService webTransportService)
        {
            ...
            var wtProposal = webTransportService.GetProposalAndProjectHeaders(proposalEstimate.ProposalNumber);
            ...
            var reportProposal = new ReportProposal
            {
                ...
                //HasOversight =
                LettingDate = wtProposal.MyLetting == null ? (DateTime?)null : wtProposal.MyLetting.LettingDate,
                LettingId = wtProposal.MyLetting == null ? string.Empty : wtProposal.MyLetting.LettingName,
            ...
            foreach (var reportProject in reportProposal.ReportProjects)
            {
                ...
                foreach (var category in estimate.EstimateGroups)
                {
                    ...
                    foreach (var item in category.ProjectItems)
                    {
                        var payItem = payItems.FirstOrDefault(i => i.RefItemName == item.PayItemNumber);
                        var reportProjectItem = new ReportProjectItem();
                        reportProjectItem.Description = item.PayItemDescription;
                        reportProjectItem.DoNotBid = payItem != null && payItem.NonBid;
                        reportProjectItem.IsLowCost = reportProposal.ReportProposalSummaries
                            .Where(i => i.CategoryAlternateSet == category.AlternateSet)
                            .Where(i => i.CategoryAlternateMember == category.AlternateMember)
                            .Where(i => i.ItemAlternateSet == item.AlternateSet)
                            .First(i => i.ItemAlternateMember == item.AlternateMember).IsLowCost;
                        reportProjectItem.ItemAlternateMember = item.AlternateMember;
                        reportProjectItem.ItemAlternateSet = item.AlternateSet;
                        reportProjectItem.ItemNumber = item.PayItemNumber;
                        reportProjectItem.Price = item.Price;
                        reportProjectItem.Quantity = item.Quantity;
                        ...
                        reportProjectItem.IsObsolete = reportProjectItem.ObsoleteDate.HasValue 
                            && reportProposal.LettingDate.HasValue 
                            && reportProjectItem.ObsoleteDate.Value.Date < reportProposal.LettingDate.Value.Date;
                        reportProjectItem.TechSpec = payItem == null ? string.Empty : payItem.Ilflg1 ?? string.Empty;
                        reportProjectItem.Unit = payItem == null
                            ? reportProjectItem.Unit
