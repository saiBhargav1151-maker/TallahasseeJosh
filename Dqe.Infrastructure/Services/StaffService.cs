...
namespace Dqe.Infrastructure.Services
{
    public class StaffService : IStaffService
    {
        ...
        public AuthenticationToken ChangePassword(string user, string password, string newPassword)
        {
            var proxy = ChannelProvider<FDOT.Enterprise.Racf.Client.IRacfService>.Default;
            var token = proxy.AuthenticateAndChangePassword(user, password, newPassword);
            return new AuthenticationToken
            {
                IsAuthenticated = token.IsAuthenticated,
                IsPasswordExpired = token.IsPasswordExpired,
