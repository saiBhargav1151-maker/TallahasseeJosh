...
namespace Dqe.Infrastructure.Services
{
    public class TaskRunner : ITaskRunner
    {
        ...
        public void CopyMasterFile(int masterFileId, DateTime effectiveDate, int currentUserSrsId, int fileNumber)
        {
            try
            {
                _deferredTaskHubContext.SendMessage(ApplicationConstants.Tasks.CopyMasterFile, "Copying Master File started.");
                ...
                using (var session = Initializer.SessionFactory.OpenSession())
                {
                    ...
                    using (var transaction = session.BeginTransaction())
                    {
                        ...
                        if (!Validator.TryValidateObject(mf, new ValidationContext(mf, null, null), validationResults)) AbortTask(validationResults, ApplicationConstants.Tasks.CopyMasterFile);
                        if (!validationResults.Any())
                        {
                            ...
                            foreach (var piCopy in copyMf.PayItems)
                            {
                                ...
                                if (piCopy.EffectiveDate.HasValue)
                                {
                                    if (piCopy.ObsoleteDate.HasValue)
                                    {
                                        ...
                                        if (piCopy.ObsoleteDate.Value.Date >= effectiveDate)
                                        {
                                            ...
                                            {
                                                AbortTask(validationResults, ApplicationConstants.Tasks.CopyMasterFile);
                                                break;
                                            }
                                        }
                                    }
                                ...
                                if (!Validator.TryValidateObject(piNew, new ValidationContext(piNew, null, null), validationResults)) AbortTask(validationResults, ApplicationConstants.Tasks.CopyMasterFile);
                            }
                        }
                        if (!validationResults.Any())
                        {
                ...
                if (!validationResults.Any()) _deferredTaskHubContext.SendMessage(ApplicationConstants.Tasks.CopyMasterFile, "Copying Master File finished.");
            }
            catch
            {
                AbortTask(new List<ValidationResult> { new ValidationResult("Application exception occurred.") }, ApplicationConstants.Tasks.CopyMasterFile);
                throw;
            }
        }

        private void AbortTask(IEnumerable<ValidationResult> validationResults, string task)
        {
            using (var session = Initializer.SessionFactory.OpenSession())
            {
                using (var transaction = session.BeginTransaction())
            ...
            _deferredTaskHubContext.SendMessage(task, sb.ToString());  
        }
    }
}