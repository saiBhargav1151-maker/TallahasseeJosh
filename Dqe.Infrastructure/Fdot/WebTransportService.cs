...
namespace Dqe.Infrastructure.Fdot
{
    public class WebTransportService : IWebTransportService
    {
        ...
        public void UpdatePrices(Domain.Model.Proposal p, DqeUser user)
        {
#if DEBUG
            return;
#endif
            ...
            using (var session = Initializer.TransportSessionFactory.OpenSession())
            {
                ...
                //var x = session.QueryOver<Proposal>().Where(i => i.Id == 24455).List();
                //var z = x.Count;
                //var x2 = session.QueryOver<Proposal>().Where(i => i.Id == 24455).SingleOrDefault();

                //var xx = session.QueryOver<Proposal>().Where(i => i.Id == 24454).List();
                //var xx2 = session.QueryOver<Proposal>().Where(i => i.Id == 24454).SingleOrDefault();

                //var xxx = session.QueryOver<Proposal>().Where(i => i.ProposalNumber == "KVTST1").List();
                ...
                queryUpdateProposalItemSb.Append("        EstimateType    = :estimateType,    ");
                ...
                var records = queryUpdateProposal
                    .SetParameter("price", total.Total)
                    .SetParameter("lastUpdatedDate", DateTime.Now)
                    .SetParameter("lastUpdatedBy", "DQE")
                    .SetParameter("id", p.WtId)
                    .ExecuteUpdate();
                if (records != 1) throw new InvalidOperationException("Updated unexpected proposal");
                ...
                foreach (var project in projects)
                {
                    ...
                    if (estimate != null)
                    {
                        ...
                        records = queryUpdateProject
                            .SetParameter("price", projectTotal.Total)
                            .SetParameter("estimatedDate", DateTime.Now)
                            .SetParameter("lastUpdatedDate", DateTime.Now)
                            .SetParameter("lastUpdatedBy", "DQE")
                            .SetParameter("pricedBy", user.Name)
                            .SetParameter("pricedDate", DateTime.Now)
                            .SetParameter("id", p.WtId)
                            .ExecuteUpdate();
                        if (records != 1) throw new InvalidOperationException("Updated unexpected project");
                        foreach (var categorySet in projectTotal.CategorySets)
                        {
                            foreach (var estimateGroup in categorySet.EstimateGroups)
                            {
                                records = queryUpdateCategory
                                    ...
                                    .ExecuteUpdate();
                                if (records != 1) throw new InvalidOperationException("Updated unexpected category");
                            }
                        }
                    }
                }
                ...
                foreach (var categorySet in total.CategorySets)
                {
                    foreach (var sectionGroup in categorySet.SectionGroups)
                    {
                        ...
                        records = queryUpdateSection
                            .SetParameter("price", sectionGroupTotal)
                            .SetParameter("isLowCost", categorySet.Included)
                            .SetParameter("isOriginalLowCost", true)
                            .SetParameter("id", sectionGroup.WtId)
                            .ExecuteUpdate();
                        if (records != 1) throw new InvalidOperationException("Updated unexpected section");
                    }
                        ...
                        foreach (var proposalItem in itemSet.ProposalItems)
                        {
                            ...
                            if (proposalItemTemp != null)
                            {
                                records = queryUpdateProposalItem
                                    .SetParameter("price", proposalItemTemp.Price)
                                    .SetParameter("isLowCost", itemSet.Included)
                                    .SetParameter("extendedAmount", proposalItem.Quantity*proposalItemTemp.Price)
                                    .SetParameter("estimateType", Enum.GetName(typeof (PriceSetType), proposalItemTemp.PriceSet))
                                    .SetParameter("lastUpdatedDate", DateTime.Now)
                                    .SetParameter("lastUpdatedBy", "DQE")
                                    .SetParameter("id", proposalItem.WtId)
                                    .ExecuteUpdate();
                                if (records != 1) throw new InvalidOperationException("Updated unexpected proposal item");
                                foreach (var projectItem in projectItems)
                                {
                                    records = queryUpdateProjectItem
                                    .SetParameter("price", projectItem.Price)
                                    .SetParameter("isLowCost", itemSet.Included)
                                    .SetParameter("lastUpdatedDate", DateTime.Now)
                                    .SetParameter("lastUpdatedBy", "DQE")
