    ...
    class Program
    {
        static void Main()
        {
            try
            {
                ...
                if (unSynchronizedProposals.Any() || noOfficialProposals.Any())
                {
        ...
        private static void HandleNoOfficialEmail(IDqeUserRepository dqeUserRepository, List<Domain.Model.Wt.Proposal> wtProposal, string environment, List<string> emailAddresses)
        {
            if (!emailAddresses.Any())
                emailAddresses = AcquireEmailAddresses(dqeUserRepository);

            var sb = new StringBuilder();
            foreach (var proposal in wtProposal)
            {
                var status = proposal.ProposalStatus == "02" ? "Awarded" : "Executed";
                sb.AppendLine("Proposal " + proposal.ProposalNumber + " was found in WT with a status of " + status + " without a corresponding record in DQE.<br />");
            }
        ...
        private static void HandleSynchronizationEmail(IDqeUserRepository dqeUserRepository, List<Proposal> proposals, string environment, List<string> emailAddresses)
        {
            if (!emailAddresses.Any())
                emailAddresses = AcquireEmailAddresses(dqeUserRepository);

            var sb = new StringBuilder();
            sb.AppendLine("The following Official Estimate Proposals were not updated in WT. <br />");
