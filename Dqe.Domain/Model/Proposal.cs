...
namespace Dqe.Domain.Model
{
    public class Proposal : Entity<Transformers.Proposal>
    {
        ...
        public virtual void SynchronizeStructure(Wt.Proposal wtp, DqeUser custodyUser)
        {
            ...
            var projectItems = _proposalRepository.GetDqeProjectItemsForProposal(custodyUser, projects).ToList();
            foreach (var s in wtp.Sections)
            {
                ...
                foreach (var i in s.ProposalItems)
                {
                    ...
                    foreach (var pi in i.ProjectItems)
                    {
                        ...
                        if (projectItem == null)
                        {
                            throw new InvalidOperationException("Project Item not found");
                        }
                    ...
                    foreach (var itemSet in itemSets)
                    {
                        ...
                        foreach (var itemMember in itemMembers)
                        {
                            ...
                            var iSet = new ItemSet
                            {
                                ...
                                Total = memberItems.Sum(i => i.GetEstimatorProjectItems(estimator).Sum(ii => ii.Quantity * ii.Price))
                            };
        ...
        public virtual IEnumerable<ItemGroup> GetItemGroups()
        {
            ...
            foreach (var proposalItem in l)
            {
                var ig = new ItemGroup
                {
                    ...
                    FederalConstructionClass = proposalItem.MySectionGroup.Name,
                    Fund = string.Empty,
