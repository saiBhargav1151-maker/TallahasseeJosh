...
namespace Dqe.Domain.Model
{
    public class ProjectEstimate : Entity<Transformers.ProjectEstimate>
    {
                    ...
                    foreach (var itemSet in itemSets)
                    {
                        ...
                        foreach (var itemMember in itemMembers)
                        {
                            ...
                            var iSet = new ItemSet
                            {
                                Set = itemSet,
                                Member = itemMember,
                                Total = memberItems.Sum(i => Math.Round(i.Quantity * i.Price, 2))
                            };
        ...
        public virtual void SyncWithWt(bool initializePrices, DqeUser account)
        {
            ...
            var newEstimate = MyProjectVersion.MyProject.CreateNewVersionFromWt("Synchronized with Web Trns*Port design changes", project, initializePrices, account);
