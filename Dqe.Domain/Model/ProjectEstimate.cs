...
namespace Dqe.Domain.Model
{
    public class ProjectEstimate : Entity<Transformers.ProjectEstimate>
    {
        ...
        public virtual DateTime? LabelRemovedOn { get; protected internal set; }

        public virtual string LabelRemovedComment { get; protected internal set; }
        ...
        public virtual IEnumerable<ItemGroup> GetItemGroups()
        {
            ...
            var combinedCategoryAndItemKeys = l.Select(i => new
            {
                ...
                i.CalculatedUnit,
                //CalculatedUnit = i.CalculatedUnit.ToUpper().StartsWith("LS")
                //? i.Unit.ToUpper().Trim() == "LS"
                //    ? i.CalculatedUnit
                //    : string.Format("{0}/{1}", i.CalculatedUnit, i.Unit)
                //: i.CalculatedUnit
            })
            ...
            foreach (var combinedCategoryAndItemKey in combinedCategoryAndItemKeys)
            {
                ...
                var ig = new ItemGroup
                {
                    ...
                    FederalConstructionClass = !items.Any()
                        ? string.Empty
                        : items.Count == 1
                            ? items.ToList()[0].MyEstimateGroup.Name
                            : string.Format("{0}XX", items.ToList()[0].MyEstimateGroup.Name.Substring(0, 2)),
                    ...
                    foreach (var itemSet in itemSets)
                    {
                        ...
                        foreach (var itemMember in itemMembers)
                        {
                            ...
                            var iSet = new ItemSet
                            {
                                Set = itemSet,
                                Member = itemMember,
                                Total = memberItems.Sum(i => Math.Round(i.Quantity * i.Price, 2, MidpointRounding.AwayFromZero))
        ...
        public virtual EstimateTotal GetEstimateTotalWithItems()
        {
            var estimateTotal = new EstimateTotal();
            var categorySets = EstimateGroups.Select(i => i.AlternateSet).Distinct().ToList();
            foreach (var categorySet in categorySets)
            {
                ...
                foreach (var categoryMember in categoryMembers)
                {
                    ...
                    var cSet = new CategorySet
                    {
                        Set = categorySet,
                        Member = categoryMember,
                        EstimateGroups = memberCategories
                    };
                    estimateTotal.CategorySets.Add(cSet);
                    var allItems = new List<ProjectItem>();
                    foreach (var category in memberCategories)
                ...
                {                  
                    foreach (var pItem in estimateGroup.ProjectItems)
                    {
                        ...
                        if (piMatch != null)
                        {
                            ...
                            piMatch.PreviousPrice = pi.PreviousPrice;
        ...
        public virtual void SetComment(string comment, DqeUser account)
        {
            ...
            if (account.Role != DqeRole.System && account.Role != DqeRole.Administrator && account.Role != DqeRole.DistrictAdministrator && account.Role != DqeRole.Estimator && account.Role != DqeRole.DistrictReviewer && account.Role != DqeRole.StateReviewer)
            {
                throw new SecurityException(string.Format("Account role {0} is not authorized for this transaction.", account.Role));
            }
            if (MyProjectVersion.VersionOwner != account)
