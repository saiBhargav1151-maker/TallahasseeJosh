...
namespace Dqe.Domain.Model
{
    public class Project : Entity<Transformers.Project>
    {
        ...
        public virtual ProjectEstimate CreateNewVersionFromWt(string comment, Wt.Estimate source, DqeUser account)
        {
            if (account == null) throw new ArgumentNullException("account");
            if (account.Role != DqeRole.System && account.Role != DqeRole.Administrator && account.Role != DqeRole.DistrictAdministrator && account.Role != DqeRole.Estimator)
            {
        ...
        public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            {
                yield return new ValidationResult(string.Format("Only one Proposal associated to Project {0} can have a type of Web Trns*Port."));
            }
        }
    }
}