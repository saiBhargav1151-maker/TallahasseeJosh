...
namespace Dqe.Domain.Model
{
    public class PayItem : Entity<Transformers.PayItem>
    {
        ...
        public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            {
                yield return new ValidationResult("A Pay Item with this Number and Master File already exists");
            }
            if (ObsoleteDate.HasValue && !EffectiveDate.HasValue)
            {
                yield return new ValidationResult("A Pay Item must have an Effective Date to have an Obsolete Date");
            }
            if (MyPayItemStructure.ObsoleteDate.HasValue && !ObsoleteDate.HasValue)
            {
                yield return new ValidationResult("A Pay Item must have an Obsolete Date if the parent Structure has an Obsolete Date");
            }
            ...
            if (isOverlap)
            {
                yield return new ValidationResult("The effective range for this Pay Item overlaps with another Pay Item with the same Number");
            }
            if (EffectiveDate.HasValue && EffectiveDate.Value.Date < MyPayItemStructure.EffectiveDate.Date)
            {
                yield return new ValidationResult("Effective Date must be equal to or greater than the Pay Item Structure's Effective Date");
            }
            ...
            {
                yield return new ValidationResult("Obsolete Date must be equal to or less than the Pay Item Structure's Obsolete Date");
            }
        }
    }
}