dqeControllers.controller('HomeBoeController', ['$scope', '$rootScope', '$http', '$route', '$filter', '$location', function ($scope, $rootScope, $http, $route, $filter, $location) {
    ...
    if ($route.current.params != 'undefined' && $route.current.params != null) {
        if ($route.current.params.index != 'undefined' && $route.current.params.index != null) {
            ...
            if (arr.length == 2) {
                ...
                if ($route.current.params.index == 'X') {
                    initialSet = 'X';
                }
                $http.get('./PayItemStructureAdministration/GetStructuresRange', { params: { set: initialSet, currentStructuresOnly: true } }).success(function (result) {
                    if (!containsDqeError(result)) {
                        $scope.structures = getDqeData(result);
                        $scope.showList = true;
                        $scope.showChapters = false;
    ...
    $scope.viewBoeReport = function () {
        var structureType = document.getElementById("hiddenStructureName").value;
        $http.get('./report/ValidateBoeParameters', { params: { structureType: structureType } }).success(function (result) {
            if (!containsDqeError(result)) {
                $.download('./report/ViewBoeReport', $('form#ViewBoeReport').serialize());
            }
        });
    };
