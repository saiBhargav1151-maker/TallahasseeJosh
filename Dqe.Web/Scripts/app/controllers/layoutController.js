dqeControllers.controller('LayoutController', ['$scope', '$rootScope', '$location', '$cookieStore', '$window', 'securityService', 'navigationService', function ($scope, $rootScope, $location, $cookieStore, $window, securityService, navigationService) {
    ...
    $scope.deferredTaskHub.client.showMessage = function (task, message) {
        ...
        if (task == 'CopyMasterFile') {
            $rootScope.$broadcast('checkMasterFileCopyInProcess');
        }
        $rootScope.$apply();
    };
