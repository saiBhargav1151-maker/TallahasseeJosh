    ...
    public class DynamicModelBinder : DefaultModelBinder
    {
        public override object BindModel(ControllerContext controllerContext, ModelBindingContext bindingContext)
        {
            ...
            using (var reader = new StreamReader(controllerContext.HttpContext.Request.InputStream))
            {
                ...
                {
                    int num;
                    ((IDictionary<string, object>)result).Add(prop.Name, prop.Value.ToObject<object>() == null?null:
                        int.TryParse(prop.Value.ToObject<object>().ToString(), out num) ? num : prop.Value.ToObject<object>());
                }
                return result;
            }
        }
