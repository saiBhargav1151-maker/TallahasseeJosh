...
namespace Dqe.Web.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        {
            const string reportFormat = "EXCELOPENXML";
            var specYear = form["specYear"];

            var targetUrl = string.Format(_serviceUrl + "/PayItemDataFile&rs:Command=Render&rs:Format={0}&MasterFileSequence={1}", reportFormat, specYear);
        ...
        public ActionResult SetupProposalSummaryReport(string proposalNumber)
        {
            ...
            if (letting != null)
            {
                ...
                if (LettingRebuildChecks(letting, dqeOfficialLetting) || LettingRebuildChecks(letting, dqeAuthorizationLetting))
                {
                    ...
                    var latest = masterFiles.Where(i => i.FileNumber < 90).Max(f => f.FileNumber);
                    var frontLoadedPayItems = _payItemMasterRepository.GetFrontLoadedPayItems(latest).ToList();

                    //delete letting data
                    _reportRepository.DeleteLettingData(letting, false);

                    //rebuild
                    _reportRepository.RebuildReportStructure(letting, null, null, false, frontLoadedPayItems);
                }
            }
            else
            {
        ...
        public ActionResult SaveLettingAndVendorDataByProposal(string proposalNumber)
        {
            ...
            if (Rebuild(letting, dqeLetting))
            {
                ...
                //rebuild
                _reportRepository.RebuildReportStructure(letting, officialProposals, null, true, frontLoadedPayItems);
            }
        ...
        public ActionResult SaveLettingAndVendorDataByLettingForEstimateTolerance(string lettingNumber)
        {
            ...
            if (Rebuild(letting, dqeLetting))
            {
                ...
                //rebuild
                _reportRepository.RebuildReportStructure(letting, officialProposals, authorizedProposals, true, frontLoadedPayItems);
            }
