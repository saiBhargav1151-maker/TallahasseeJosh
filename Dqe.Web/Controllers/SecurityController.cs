...
namespace Dqe.Web.Controllers
{
    ...
    public class SecurityController : Controller
    {
        ...
        public ActionResult AuthenticateUser(dynamic user)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(user.newPassword) || string.IsNullOrWhiteSpace(user.confirmPassword))
                {
                    return new DqeResult(null, new ClientMessage { Severity = ClientMessageSeverity.Error, text = "New password and password confirmation are required" });
                }
                if (user.newPassword != user.confirmPassword)
                {
                    return new DqeResult(null, new ClientMessage { Severity = ClientMessageSeverity.Error, text = "New password and password confirmation do not match" });
                }

            }
            var token = user.isChangePassword 
                ? (AuthenticationToken)_staffService.ChangePassword(user.id, user.password, user.newPassword)
                : (AuthenticationToken)_staffService.AuthenticateUser(user.id, user.password);
#if !DEBUG
