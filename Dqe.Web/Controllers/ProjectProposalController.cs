...
namespace Dqe.Web.Controllers
{
    ...
    public class ProjectProposalController : Controller
    {
        ...
        public ActionResult DeleteProposal(dynamic proposal)
        {
            ...
            if (p != null)
            {
                var workingEstimate = _reportRepository.GetReportProposal(p.ProposalNumber, ReportProposalLevel.WorkingEstimate);
        ...
        private ActionResult ResultStructureFromProposal(Proposal prop, DqeUser currentDqeUser, string successMessage)
        {
            ...
            {
                authorizationTotal = _reportRepository.GetReportProposal(prop.ProposalNumber, ReportProposalLevel.Authorization).Total;

                if (currentSnapshot == SnapshotLabel.Official)
                    officialTotal = _reportRepository.GetReportProposal(prop.ProposalNumber, ReportProposalLevel.Official).Total;
            }
        ...
        [HttpPost]
        public ActionResult BuildProposalReportData(dynamic proposal)
        {
            if (!proposal.takeLabeledSnapshot) return new DqeResult(null);
            var currentUser = (DqeIdentity)User.Identity;
            var currentDqeUser = _dqeUserRepository.GetBySrsId(currentUser.SrsId);
            _proposalRepository.BuildReportProposal(proposal.id, currentDqeUser, _payItemMasterRepository, _reportRepository, _webTransportService);
            return new DqeResult(null);
        }
        ...
        public ActionResult SnapshotProposalWorkingEstimate(dynamic project)
        {
            ...
            p.SnapshotWorkingEstimate(currentDqeUser, DynamicHelper.HasNotNullProperty(project, "takeLabeledSnapshot") && project.takeLabeledSnapshot, project.comment, true);
        ...
        [HttpGet]
        public ActionResult GetDqeProposals(string proposalNumber, int estimateType)
        {
            var proposals = _proposalRepository.GetProposalByEstimateTypeAndCategory(proposalNumber, estimateType);
            return Json(proposals
                .Select(i => new
                {
                    id = i.Id,
