            ...
            if (lreProjects.Count > 1)
            {
                ...
                if (lreProject == null)
                {
                    return new DqeResult(null,
                        ...
                        {
                            Severity = ClientMessageSeverity.Error,
                            text = string.Format("LRE project {0} has been created in a different district than the Preconstruction assigned District.  The District assignments must match to continue.", number)
                        },
                        JsonRequestBehavior.AllowGet);
                }
            }
