...
namespace Dqe.Web.Controllers
{
    ...
    public class MasterFileAdministrationController : Controller
    {
        ...
        [HttpPost]
        [CustomAuthorize(Roles = new[] { DqeRole.Administrator })]
        public ActionResult UpdateMasterFile(dynamic masterFile)
        {
            var currentUser = (DqeIdentity)User.Identity;
            var currentDqeUser = _dqeUserRepository.GetBySrsId(currentUser.SrsId);

            if (masterFile == null || masterFile.specBook == null)
                return new DqeResult(null, new ClientMessage { Severity = ClientMessageSeverity.Error, text = "Master File number is required" });

            if (masterFile.effectiveDate == null)
                return new DqeResult(null, new ClientMessage { Severity = ClientMessageSeverity.Error, text = "Master File opened date is required" });

            if (((DateTime)masterFile.effectiveDate).Date < DateTime.Now.Date)
                return new DqeResult(null, new ClientMessage { Severity = ClientMessageSeverity.Error, text = "Master File opened date cannot be prior to today" });

            var masterFiles = _masterFileRepository.GetAll();

            if (masterFiles.Any(i => i.FileNumber == masterFile.specBook))
                return new DqeResult(null, new ClientMessage { Severity = ClientMessageSeverity.Error, text = "There is already a Master File for the provided pay item year." });

            var mf = (MasterFile)_masterFileRepository.Get(masterFile.id);

            var mft = mf.GetTransformer();
            mft.DoMasterFileCopy = masterFile.copyMasterFile;
            mft.EffectiveDate = masterFile.effectiveDate;
            mf.Transform(mft, currentDqeUser);

            return new DqeResult(masterFile, new ClientMessage { Severity = ClientMessageSeverity.Success, text = "Master File updated" }, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        [CustomAuthorize(Roles = new[] { DqeRole.Administrator })]
        public ActionResult CancelMasterFile(dynamic masterFile)
        {
            var currentUser = (DqeIdentity)User.Identity;
            var currentDqeUser = _dqeUserRepository.GetBySrsId(currentUser.SrsId);
            ...
            var mft = mf.GetTransformer();
            mft.DoMasterFileCopy = false;
            mft.EffectiveDate = null;
            mf.Transform(mft, currentDqeUser);

            return new DqeResult(masterFile, new ClientMessage { Severity = ClientMessageSeverity.Success, text = "New Master File updated will not be created" }, JsonRequestBehavior.AllowGet);
        }
