    ...
    public class CostBasedTemplateAdministrationController : Controller
    {
        ...
        public ActionResult GetAll()
        {
            ...
            .Select(i =>
                new
                {
                    ...
                    documentId = i.CurrentDocumentVersion.EdmsDocumentId
        ...
        public ActionResult SaveCostBasedTemplate()
        {
            ...
            if (Request.Files != null && Request.Files.Count > 0 && Request.Files[0] != null)
            {
                ...
                {
                    return new DqeResult(new object(), new ClientMessage { Severity = ClientMessageSeverity.Error, text = "The uploaded file must be a .xlsx excel file" });
                }
                templateDocument = _documentService.SaveDocumentToEdms(Request.Files[0].FileName, Request.Files[0].InputStream, _documentService.GetCostBasedTemplateDocumentType());
            }
            ...
            if (templateDocument != null)
            {
                ...
                docTransformer.EdmsDocumentId = templateDocument.EdmsId;
            ...
            if (validationResults.Any())
            {
                ...
                return new DqeResult(null, validationResults.Select(v => new ClientMessage { Severity = ClientMessageSeverity.Error, text = v.ErrorMessage }));
            }
            string successMsg;
            if (id == 0)
            {
            ...
            {
                name = template.Name, 
                resultCell = template.ResultCell, 
                id = template.Id, 
                documentId = template.CurrentDocumentVersion.Id
        ...
        {
            var cbt = _costBasedTemplateRepository.Get(id);
            var document = _documentService.GetDocumentContent(cbt.CurrentDocumentVersion.EdmsDocumentId);
            return File(document, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", string.Format("{0}.xlsx", cbt.Name));
        }

        [HttpPost]
        [CustomAuthorize(Roles = new[] { DqeRole.Administrator, DqeRole.CostBasedTemplateAdministrator })]
        public ActionResult RemoveCostBasedTemplates(dynamic template)
        {
            var t = _costBasedTemplateRepository.Get(template.id);
            if (t != null) _commandRepository.Remove(t);
            return new DqeResult(null, new ClientMessage { text = "Cost-Based Template Removed" });
        }
    }
}