...
namespace Dqe.Web.Controllers
{
    ...
    public class PayItemStructureAdministrationController : Controller
    {
        ...
        {
            var structures = _payItemStructureRepository.GetAllHeaders(string.Empty);
        ...
        [HttpGet]
        public ActionResult GetStructuresRange(string set)
        {
            var structures = _payItemStructureRepository.GetAllHeaders(set);
            var result = structures.Select(i => new
            {
                id = i.Id,
                name = string.IsNullOrWhiteSpace(i.StructureId) ? string.Empty : i.StructureId,
        ...
        public ActionResult GetPayItemsBySpecBook(string specBook)
        {
            ...
            var payItems = structures.Select(i => new
            {
                ...
                openDate = i.OpenedDate.HasValue ? i.OpenedDate.Value.ToShortDateString() : string.Empty,
        ...
        public ActionResult SaveItem(dynamic item)
        {
            ...
            if (item.id == 0)
            {
                var cps = _payItemMasterRepository.GetByName((string)item.name).ToList();
            ...
            else
            {
                ...
                var cps = _payItemMasterRepository.GetByName((string)item.name).Where(i => i.Id != tp.Id).ToList();
            ...
            if (pi.Id == 0)
            {
                ...
                _webTransportService.UpdateRefItem(pi, true, currentDqeUser);
            ...
            _webTransportService.UpdateRefItem(pi, false, currentDqeUser);
