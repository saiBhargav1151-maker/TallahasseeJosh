    ...
    public class PayItemStructureAdministrationController : Controller
    {
        ...
        public ActionResult SaveItem(dynamic item)
        {
            ...
            if (item.id == 0)
            {
                ...
                {
                    return new DqeResult(null, new ClientMessage { Severity = ClientMessageSeverity.Error, text = "There is an existing Pay Item with this ID and pay item year" });
                }
        ...
        public ActionResult SaveStructure(dynamic structure)
        {
            ...
            if (structure.id == 0)
            {
                var s = (PayItemStructure)_payItemStructureRepository.GetByStructureId(structure.name.ToString(), null);
                if (s != null)
                {
            ...
            pist.RequiredItems = structure.requiredItems.ToString();
            pist.RecommendedItems = structure.recommendedItems.ToString();
            pist.ReplacementItems = structure.replacementItems.ToString();
