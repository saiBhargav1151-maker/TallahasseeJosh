...
namespace Dqe.Web.ActionResults
{
    public class DqeResult : JsonResult
    {
        ...
        {
            dynamic wrapper = new DqeResultObject();

            var messages=new Dictionary<ClientMessageSeverity, ClientMessage>();
            foreach (var message in _clientMessages)
            {
                if (!messages.ContainsKey(message.Severity))
                {
                    messages.Add(message.Severity,message);
                }
                else
                {
                    messages[message.Severity].text += "<br/>" +message.text;
                }
            }
            wrapper.messages = messages.Select(m=>m.Value);
            wrapper.data = _data;
            var result = new JsonResult
            {
                ContentEncoding = _encoding,
                ContentType = _contentType,
                Data = _clientMessages.Any() ? wrapper : _data,
