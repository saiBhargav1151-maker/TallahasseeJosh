...
namespace Dqe.Web.Hubs
{
    public class DeferredTaskHubContext : IDeferredTaskHubContext
    {
        ...
        public void SendMessageToUser(string user, string task, string message)
        {
            GlobalHost.ConnectionManager.GetHubContext<DeferredTaskHub>().Clients.User(user).showMessage(task, message);
        }
    }
}